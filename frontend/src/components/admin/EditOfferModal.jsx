...
export default function EditOfferModal({ item, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/offers/${item._id}`, {
        method: 'PUT',
        headers: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      ...
      >
        ...
        <h2 className="text-2xl font-heading text-white mb-6 uppercase tracking-wider">Edit Offer</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-neutral-400 mb-1">Replace Media (Optional)</label>
            <input
              type="file"
              onChange={(e) => setFile(e.target.files[0])}
              className="w-full bg-black/50 border border-neutral-700 py-2 px-3 text-neutral-400 file:bg-neutral-800 file:text-white file:border-0 file:py-1 file:px-3 file:mr-4 file:font-medium transition-colors focus:outline-none"
            />
